...
def test_handle_voice_message_resends(monkeypatch, tmp_path):
    ...
    msg = SimpleNamespace(
        ...
        voice=SimpleNamespace(file_id="fid", file_unique_id="uid", duration=35),
        message_id=5,
        date=1,
    )
...
def test_handle_voice_too_short(monkeypatch, tmp_path):
    db_file = tmp_path / "bot.db"
    monkeypatch.setenv("DB_PATH", str(db_file))
    ...
    monkeypatch.setattr(wsh, "RESPONSES_DIR", str(tmp_path / "resp"))
    monkeypatch.setattr(vh, "RESPONSES_DIR", str(tmp_path / "resp"))
    monkeypatch.setattr(wsh, "survey_menu", lambda uid, qi, vc=0: "menu")
    monkeypatch.setattr(wsh, "get_translation", lambda uid, key: key)
    monkeypatch.setattr(vh, "get_translation", lambda uid, key: key)
    ...
    ss.SurveyManager.get_session(1)

    msg = SimpleNamespace(
        chat=SimpleNamespace(id=1),
        voice=SimpleNamespace(file_id="fid", file_unique_id="uid", duration=10),
        message_id=5,
        date=1,
    )
    ...
    assert bot.deleted == [5]
    assert not bot.sent_voice
    assert "voice_too_short" in bot.sent_text
    conn = db.get_connection()
...
def test_handle_voice_message_multiple_resend_all(monkeypatch, tmp_path):
    ...
    msg2 = SimpleNamespace(
        chat=SimpleNamespace(id=1),
        voice=SimpleNamespace(file_id="fid2", file_unique_id="uid2", duration=40),
        message_id=6,
        date=2,
    )
